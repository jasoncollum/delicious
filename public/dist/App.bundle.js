    ...
    _axios2.default.get('/api/stores/near?lat=' + lat + '&lng=' + lng).then(function (res) {
        ...
        var markers = places.map(function (place) {
            var _place$location$coord = _slicedToArray(place.location.coordinates, 2),
                ...
                placeLat = _place$location$coord[1]; // db: lng lat


            var position = { lat: placeLat, lng: placeLng // google: lat lng
            };bounds.extend(position); // sets bounds to include each marker
            var marker = new google.maps.Marker({ map: map, position: position }); //marker goes: (on the map, at the position)
            marker.place = place;
            return marker;
        });
...
function makeMap(mapDiv) {
    if (!mapDiv) return;
    // make map
    var map = new google.maps.Map(mapDiv, mapOptions); // (where map goes, options)
    loadPlaces(map);
...
function typeAhead(search) {
    ...
    searchInput.on('input', function () {
        ...
        searchResults.innerHTML = ''; // clear if backspace - no matches
...
heartForms.on('submit', _heart2.default); // listening for events on node list
...
function ajaxHeart(e) {
    ...
    e.preventDefault();
    _axios2.default.post(this.action) // this = the heartForm that was clicked
    .then(function (res) {
        var isHearted = _this.heart.classList.toggle('heart__button--hearted'); // heart is a sub element of heartForm
        (0, _bling.$)('.heart-count').textContent = res.data.hearts.length; // update heart count in navbar
        if (isHearted) {
            _this.heart.classList.add('heart__button--float'); // float adds scss animation
            setTimeout(function () {
